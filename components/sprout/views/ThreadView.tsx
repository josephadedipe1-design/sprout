...
export default function ThreadView({ postId, onBack }: ThreadViewProps) {
  ...
  const loadThread = useCallback(async () => {
    ...
    setLoadError('');
    const [postRes, repliesRes, likeRes, myLike] = await Promise.all([
    ...
    const userIds = Array.from(new Set([
      ...
      ...((repliesRes.data ?? []) as any[]).map((r: any) => r.author_id),
    ].filter(Boolean)));
    ...
    setReplies(
      ((repliesRes.data ?? []) as any[]).map((r: any) => ({
        id: r.id,
        body: r.body,
    ...
    <div className="max-w-2xl mx-auto px-4 py-6 pb-32 lg:pb-6">
      ...
      {!loading && post && (
        <>
          ...
          {/* Replies */}
          <div className="mb-4">
            <h3 className="text-sm font-semibold mb-3" style={{ color: '#7a6055' }}>{replies.length} {replies.length === 1 ? 'Reply' : 'Replies'}</h3>
            {replies.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-sm" style={{ color: '#9a8070' }}>No replies yet. Be the first to reply!</p>
              </div>
            ) : (
