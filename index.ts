...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!RESEND_API_KEY) {
      console.error("[send-connection-accepted] RESEND_API_KEY not configured");
    ...
    // The email goes to from_user_id (the person who sent the original request)
    // to let them know their request was accepted by to_user_id
    const recipientId = from_user_id;

    // 1. Check notification preferences — only send if email_connections is true
    ...
    if (recipientError || !recipientUser?.user?.email) {
      console.error("[send-connection-accepted] Could not fetch recipient email:", recipientError?.message);
    ...
    // Get the accepter's name for the email
    ...
    const accepterName = accepterProfile?.first_name
      ? (accepterProfile.last_initial ? `${accepterProfile.first_name} ${accepterProfile.last_initial}.` : accepterProfile.first_name)
      : "Someone";

    // 3. Build email
    const subject = "Your connection request was accepted on Sprout Village";
    const bodyContent = `
      <h1 style="font-size:20px;font-weight:700;color:#2a1f18;margin:0 0 12px 0;">Your connection request was accepted 🎉</h1>
      <p style="font-size:15px;line-height:1.6;color:#5a4035;margin:0 0 16px 0;">
        Hi ${recipientName}, ${accepterName} accepted your connection request! You're now connected on Sprout Village.
      </p>
      <p style="font-size:15px;line-height:1.6;color:#5a4035;margin:0 0 24px 0;">
        You can now message each other and start building your local parenting village.
      </p>
      <a href="${APP_URL}" style="display:inline-block;background-color:#7D3C1A;color:#ffffff;font-size:15px;font-weight:600;padding:14px 32px;border-radius:12px;text-decoration:none;">Open Sprout Village</a>`;
    ...
    const resendRes = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject,
        html: emailTemplate("Your connection request was accepted", bodyContent),
      }),
    });

    if (!resendRes.ok) {
      const errText = await resendRes.text();
      console.error("[send-connection-accepted] Resend API error:", errText);
    ...
    console.error("[send-connection-accepted] Unhandled error:", err.message);
