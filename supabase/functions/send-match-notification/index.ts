...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!RESEND_API_KEY) {
      console.error("[send-match-notification] RESEND_API_KEY not configured");
    ...
    // 1. Check notification preferences — only send if email_matches is true
    ...
    const { data: recipientUser, error: recipientError } = await supabase.auth.admin.getUserById(to_user_id);
    if (recipientError || !recipientUser?.user?.email) {
      console.error("[send-match-notification] Could not fetch recipient email:", recipientError?.message);
    ...
    // 3. Build email
    const subject = "Someone nearby wants to connect on Sprout Village";
    const bodyContent = `
      <h1 style="font-size:20px;font-weight:700;color:#2a1f18;margin:0 0 12px 0;">Someone nearby wants to connect 🤝</h1>
      <p style="font-size:15px;line-height:1.6;color:#5a4035;margin:0 0 24px 0;">
        Hi ${recipientName}, a parent in your area saw your profile and would love to connect with you on Sprout Village.
      </p>
      <p style="font-size:15px;line-height:1.6;color:#5a4035;margin:0 0 24px 0;">
        Head to the app to see their request and start chatting.
      </p>
      <a href="${APP_URL}" style="display:inline-block;background-color:#7D3C1A;color:#ffffff;font-size:15px;font-weight:600;padding:14px 32px;border-radius:12px;text-decoration:none;">View connection requests</a>`;
    ...
    const resendRes = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject,
        html: emailTemplate("Someone nearby wants to connect", bodyContent),
      }),
    });

    if (!resendRes.ok) {
      const errText = await resendRes.text();
      console.error("[send-match-notification] Resend API error:", errText);
    ...
    console.error("[send-match-notification] Unhandled error:", err.message);
